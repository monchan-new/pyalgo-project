...
while True:
    ...
    if len(dr) > min_length: # already prepared undefined next dr-bar
        min_length += 1
        dr['momentum'] = np.sign(dr['returns'].rolling(mom).mean())
        print('\n' + '=' * 51)
        print(f'NEW SIGNAL | {datetime.datetime.now()}')
        print('=' * 51)
        print(dr.iloc[:-1].tail()) # automatically exludes undefined next dr-bar
        if dr['momentum'].iloc[-2] == 1.0: # manually exludes undefined next dr-bar
